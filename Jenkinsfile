...
cloudEnvironments.each { environName, environValues ->
  ...
  builders[podName] = {
    ...
    deployOpenShiftTemplate(containersWithProps: containers, openshift_namespace: 'kubevirt', podName: podName,
      ...
      ciPipeline(buildPrefix: 'kubevirt-image-builder', decorateBuild: decoratePRBuild(), timeout: 120) {
        ...
        try {
          ...
          stage("${environName}-lab1") {
            executeInContainer(containerName: 'ansible-executor', containerScript: "sh shell/lab.sh lab1", stageVars: params, credentials: credentials)
          }

          stage("${environName}-lab2") {
            executeInContainer(containerName: 'ansible-executor', containerScript: "sh shell/lab.sh lab2", stageVars: params, credentials: credentials)
          }
